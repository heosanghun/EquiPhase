...
def main():
    print("========================================================")
    print("WBCBENCH 2026 PATIENT-LEVEL LEAKAGE AUDIT (BL CLASS)")
    print("========================================================\n")
    ...
    # X contains both background (0-2) and cell features (3-9).
    # Since background color represents patient identity (group) and BL is patient-specific,
    # we expect leakage to be detected (high Random AUROC, lower Disjoint AUROC).
    audit_res = upaf.audit(
        ...
        n_seeds=5,
        task_name="wbc_patient_leakage_bl"
    )
    
    # Save audit results
    save_path = "data/wbc_audit_results_bl.json"
    import json
    with open(save_path, "w") as f:
        json.dump(audit_res, f, indent=4)
    print(f"WBC BL Audit results saved to {save_path}")
