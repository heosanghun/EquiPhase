...
def compute_spectral_radius(cell_forward, z_k, X_pooled, lam_eff, X_mut, X_wt_res=None, num_power_iters=10, use_autograd=True):
    if X_wt_res is None:
        X_wt_res = X_pooled
    ...
    # 1. Run standard real power iteration to project random vector into 2D dominant subspace
    v = torch.randn_like(z_k)
    v = v / torch.norm(v, p=2, dim=-1, keepdim=True).clamp(min=1e-8)
    
    if use_autograd:
        def f_single(z):
            return cell_forward(z, X_pooled, lam_eff, X_mut, X_wt_res)
            
        for _ in range(num_power_iters - 2):
            _, w = torch.autograd.functional.jvp(f_single, (z_k,), (v,))
            v = w / torch.norm(w, p=2, dim=-1, keepdim=True).clamp(min=1e-8)
            
        v0 = v
        _, v1 = torch.autograd.functional.jvp(f_single, (z_k,), (v0,))
        _, v2 = torch.autograd.functional.jvp(f_single, (z_k,), (v1,))
    else:
