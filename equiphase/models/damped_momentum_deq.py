...
class DampedMomentumDEQ(ImplicitStabilitySpectroscopy):
    ...
    - Momentum-gated damping: gated_damping = True, gamma_0 = 0.20, p_thresh = 0.1
    """
    def __init__(self, esm_dim=1280, latent_dim=64, num_starts=2, dt=0.05, damping=0.2, gated_damping=False, gamma_0=0.20, p_thresh=0.1):
    ...
    def cell_forward(self, z, X_pooled, lam_eff, X_mut=None, X_wt_res=None):
        ...
        # Determine damping factor: constant or momentum-gated
        if self.gated_damping:
            p_norm = torch.norm(p_uncut, p=2, dim=-1, keepdim=True) # (N, 1)
            gate = torch.sigmoid((self.p_thresh - p_norm) * 10.0)
            eff_damping = self.gamma_0 * gate
        else:
