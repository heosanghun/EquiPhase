...
RESULTS_PATH = os.path.join(BASE_DIR, "locked_test_results_phase5.json")
...
def train_mlp_seed(seed, train_loader, val_loader, device, run_id, config_hash):
    ...
    # R3 integrity check (use test run_id to distinguish from val run_id)
    check_and_register_run(run_id, config_hash, {"AUPRC_val": best_val_auprc}, training_loss_trace=loss_trace)
    return model
...
def main():
    ...
    df_test = pd.read_csv(os.path.join(DATA_DIR, "test_phase5.tsv"), sep="\t")
    ...
    test_dataset = LLPSResidueDataset(df_test, residue_embs, biophys_features, normalize_conditions)
    ...
    for seed in SEEDS:
        ...
        run_id = f"phase5_mlp_test_seed{seed}"
        ...
        test_preds = []
        with torch.no_grad():
            for batch_emb, batch_mask, batch_bio, batch_cond, _ in test_loader:
                batch_emb, batch_mask = batch_emb.to(device), batch_mask.to(device)
                batch_bio, batch_cond = batch_bio.to(device), batch_cond.to(device)
                pred, _, _ = model(batch_emb, batch_mask, batch_bio, batch_cond)
                test_preds.extend(pred.cpu().numpy())
        mlp_test_preds.append(np.array(test_preds))
        
        # Extract attention-pooled embeddings to train XGBoost
        train_pooled = []
        train_labels = []
        with torch.no_grad():
        ...
        with torch.no_grad():
            for batch_emb, batch_mask, batch_bio, batch_cond, _ in test_loader:
                ...
                _, pooled, _ = model(batch_emb, batch_mask, batch_bio, batch_cond)
                X_batch = torch.cat([pooled, batch_bio, batch_cond], dim=1)
                test_pooled.append(X_batch.cpu().numpy())
        X_test_xgb = np.concatenate(test_pooled, axis=0)
        
        # Train Tab-Monotone XGBoost
        constraints = [0] * 1294
        constraints[1290] = 1 # Monotonic constraint on solute concentration
        ...
        xgb_test_preds.append(preds_xgb)
        
    print("\nTraining complete. Running bootstrap evaluation on locked test set...")
    
    # Average predictions over seeds
    avg_mlp_preds = np.mean(mlp_test_preds, axis=0)
    avg_xgb_preds = np.mean(xgb_test_preds, axis=0)
    
    df_mlp = pd.DataFrame({
        "cluster_id": df_test["cluster_id"],
        "label": df_test["label"],
        "pred": avg_mlp_preds
    })
    ...
    # Check if they clear the locked test no-skill baseline (0.6448)
    base_rate = float(df_test['label'].mean())
    print(f"\nLocked Test No-Skill Baseline: {base_rate:.4f}")
    ...
    results_output = {
        "run_metadata": {
            "run_id": "run_phase5_h2_test_20260612",
            "config_hash": config_hash
        },
        "locked_test_no_skill_baseline": base_rate,
        "models": {
            "AttentionMLP": mlp_ci,
    ...
    print(f"Locked test results successfully written to {RESULTS_PATH}")
