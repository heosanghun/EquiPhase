...
im = plt.imshow(log_h, extent=extent, origin='lower', cmap='magma', aspect='equal')
cb = plt.colorbar(im, label='Log10 Data Density (750k frames)')

# Plot 5 attractors
for att in attractors:
    if att["hollow"]:
        plt.scatter(att["phi"], att["psi"], c='none', edgecolors=att["color"], marker=att["marker"], s=180, linewidths=2.5, label=att["name"], zorder=5)
    else:
        plt.scatter(att["phi"], att["psi"], c=att["color"], edgecolors='white', marker=att["marker"], s=140, linewidths=1.5, label=att["name"], zorder=5)

plt.xlim(-180, 180)
plt.ylim(-180, 180)
plt.xlabel(r'$\phi$ (degrees)', fontsize=12)
plt.ylabel(r'$\psi$ (degrees)', fontsize=12)
plt.title('Alanine Dipeptide MD Density & EquiPhase Attractors (Seed 7777)', fontsize=13, pad=12)
plt.legend(loc='lower right', framealpha=0.85, fontsize=10)
plt.grid(True, linestyle='--', alpha=0.3)

out_file = os.path.join("paper3_iclr", "fig1_ala2_density_attractors_v2.png")
plt.tight_layout()
plt.savefig(out_file, dpi=300)
plt.close()

# Quarantine old file
old_file = os.path.join("paper3_iclr", "fig1_ala2_density_attractors.png")
if os.path.exists(old_file):
    shutil.move(old_file, os.path.join("quarantine", "fig1_ala2_density_attractors_v1.png"))

h_val = hashlib.sha256(open(out_file, "rb").read()).hexdigest().upper()
print(f"Fig 1 v2 generated successfully at {out_file} | SHA256={h_val}")
