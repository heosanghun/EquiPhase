...
cb = plt.colorbar(im, label=r'Log$_{10}$ Data Density (750k frames)')
...
plt.title(r'Alanine Dipeptide MD Density & EquiPhase Attractors (Seed 7777)', fontsize=13, pad=12)
plt.legend(loc='lower right', framealpha=0.85, fontsize=10)
plt.grid(True, linestyle='--', alpha=0.3)

out_file = os.path.join("paper3_iclr", "fig1_ala2_density_attractors_v3.png")
plt.tight_layout()
plt.savefig(out_file, dpi=300)
plt.close()

h_val = hashlib.sha256(open(out_file, "rb").read()).hexdigest().upper()
print(f"Fig 1 v3 generated successfully at {out_file} | SHA256={h_val}")
