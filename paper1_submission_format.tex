...
Auditing the multi-species intrinsically disordered protein dataset (\texttt{val.tsv} $n=656$; \texttt{train.tsv} $n=2539$) established exact species distributions and baseline sequence length confound AUROCs.
...
Auditing the combined UCI Heart Disease dataset ($n=920$, 4 clinical sites) under Leave-One-Site-Out (LOSO) validation yielded a mean holdout AUROC of $0.7191 \pm 0.0814$ (Cleveland 0.7902, Hungarian 0.7956, VA Long Beach 0.6939, Switzerland 0.5967). Demographic age baseline gain was $\Delta = +0.1287$ over the age baseline ($0.5904 \pm 0.0340$).
...
\begin{enumerate}
    ...
    \item \textbf{Deep Equilibrium Network Specifications}: Pattern 2 static log literals, Pattern 14 IFT unrolling deviation, and Pattern 13 reporting transmission channel vulnerability.
\end{enumerate}

\section{Conclusion}
UPAF provides a practical blueprint for identifying baseline confounders, structural alignment artifacts, and dataset leakage in AI for Science benchmarks.
