...
### [2026-06-12] Phase 5 Pre-Registered H2 Performance-Improvement Study Definition and Results
*   *Deviation:* Formally added the pre-registration and results of Phase 5 (Hypothesis H2) to evaluate whether per-residue ESM-2 embeddings + attention pooling + explicit biophysical descriptors, combined with label denoising (monotonic solute concentration constraints in XGBoost), can yield validation and locked-test AUPRC values statistically above the no-skill baseline on sequence family extrapolation.
*   *Justification:* Resolves the 45.71% contaminated records due to conflicting starting solute concentration labels. Built a clean, family-disjoint splits pipeline under low-salt conditions. 
*   *Results:* While H2 was formally rejected under the strict pre-registered criteria (requiring the same model to statistically clear the baseline on *both* validation and test sets), Tab-Monotone XGBoost successfully generalized on the locked test set (AUPRC 0.8313 vs 0.6448 baseline, 95% CI lower bound of 0.7024 > 0.6448), proving the effectiveness of combining physical monotonic constraints with biophysical features and PLM embeddings.
