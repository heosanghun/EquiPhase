...
During manuscript compilation, four subgroup AUROC values were reported that did not correspond to any execution log output (manuscript 0.6291/0.5756 vs log 0.5898/0.6173), and an accompanying significance claim ($p < 0.05$) was asserted for a difference that the actual data does not support ($\Delta = 0.0275 < \text{MDD} = 0.0465$). The discrepancy was detected through direct line-by-line comparison against the raw execution log. This motivated establishing an automated number provenance matrix (`manuscript_number_provenance.csv`) and mandatory cross-verification scripts (`verify_manuscript_provenance_strict.py`) to prevent post-hoc report drift.
...
7. **Numerical Provenance Verification**: All quantitative values in reports and manuscripts must be extracted directly from raw execution log outputs via automated grep scripts and verified against a 1-to-1 provenance matrix.
