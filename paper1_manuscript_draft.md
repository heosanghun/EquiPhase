...
- **Training Split ($n=2539$ valid headers)**: 52.54% *Homo sapiens* (1334 proteins: 1297 standard space + 37 non-breaking space), 47.46% non-human across 32 model organisms (1205 proteins: SARS-CoV-2 9.97% [$n=253$], *E. coli* 4.81%, *C. elegans* 4.14%, Yeast 3.66%, *M. musculus* 2.72%, *D. melanogaster* 2.25%, etc.). Note: 15 positive rows in the raw file contain free text `'unknown'` in place of amino acid sequences and were excluded under the adjudicated `UNK_TEXT` policy, matching the exact ledger census ($n=2539$).
...
To test whether sequence length bias varies across taxonomic boundaries, we performed a stratified interaction audit across species strata within both splits (anchored by `claude_paper1_sec41_recompute_v3.py`, SHA-256 `925a2433...`):
- **Training Split (`train.tsv` $n=2539$)**:
  - `Homo sapiens` Subset ($n=1334$): AUROC = **`0.6323`** [0.6019, 0.6636] (Bootstrap SE = $0.0158$, Hanley-McNeil SE = $0.0157$)
  - Non-Human Subset ($n=1205$): AUROC = **`0.5698`** [0.5358, 0.6039] (Bootstrap SE = $0.0174$, Hanley-McNeil SE = $0.0174$)
  - Viral `SARS-CoV-2` Subset ($n=253$): AUROC = **`0.5839`** [0.5211, 0.6418] (Bootstrap SE = $0.0312$, Hanley-McNeil SE = $0.0385$)
  - Interaction Test: $\Delta = A_{\text{nonhuman}} - A_{\text{human}} = \mathbf{-0.0626}$, $\text{SE}_{\text{diff}} = 0.0235$, $\text{MDD} = 0.0461$. Because $|\Delta| > \text{MDD}$, the training split exhibits a statistically significant taxonomic interaction ($p < 0.05$), where sequence length bias is significantly stronger in human proteins than non-human proteins.
- **Validation Split (`val.tsv` $n=656$)**:
  - `Homo sapiens` Subset ($n=483$): AUROC = **`0.5948`** [0.5408, 0.6455] (Bootstrap SE = $0.0268$)
  - Non-Human Subset ($n=173$): AUROC = **`0.6076`** [0.5148, 0.7006] (Bootstrap SE = $0.0473$)
  - Interaction Test: $\Delta = +0.0128$, $\text{SE}_{\text{diff}} = 0.0544$, $\text{MDD} = 0.1066$. The validation split exhibits non-significant interaction ($|\Delta| < \text{MDD}$) due to lower non-human sample size ($n=173$) and compositional shift across splits (73.6% vs 52.5% human).
...
| :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **Validation Split (`val.tsv`)** | Pure AA Sequence Length (`CONF_seqlen`) | $656$ | $441$ ($67.2\%$) | $215$ ($32.8\%$) | **`0.6017`** | `[0.5566, 0.6470]` (Bootstrap SE = $0.0234$) | **Statistically Significant** |
...
| **Validation Strata: Human** | `Homo sapiens` Subset (`CONF_seqlen`) | $483$ | $321$ ($66.5\%$) | $162$ ($33.5\%$) | **`0.5948`** | `[0.5408, 0.6455]` (Bootstrap SE = $0.0268$) | **Statistically Significant** |
| **Validation Strata: Non-Human** | Non-Human Subset (`CONF_seqlen`) | $173$ | $120$ ($69.4\%$) | $53$ ($30.6\%$) | **`0.6076`** | `[0.5148, 0.7006]` (Bootstrap SE = $0.0473$) | **Statistically Significant** |
| **Training Split (`train.tsv`)** | Pure AA Sequence Length (`CONF_seqlen`) | $2539$ | $1734$ ($68.3\%$) | $805$ ($31.7\%$) | **`0.6010`** | `[0.5788, 0.6236]` (Bootstrap SE = $0.0116$) | **Statistically Significant** |
...
| **Training Strata: Human** | `Homo sapiens` Subset (`CONF_seqlen`) | $1334$ | $911$ ($68.3\%$) | $423$ ($31.7\%$) | **`0.6323`** | `[0.6019, 0.6636]` (Bootstrap SE = $0.0158$) | **Statistically Significant** |
| **Training Strata: Non-Human** | Non-Human Subset (`CONF_seqlen`) | $1205$ | $823$ ($68.3\%$) | $382$ ($31.7\%$) | **`0.5698`** | `[0.5358, 0.6039]` (Bootstrap SE = $0.0174$) | **Statistically Significant** |
| **Training Strata: Viral** | SARS-CoV-2 Subset (`CONF_seqlen`) | $253$ | $180$ ($71.1\%$) | $73$ ($28.9\%$) | **`0.5839`** | `[0.5211, 0.6418]` (Bootstrap SE = $0.0312$) | Uninformative Noise ($0.50 \in \text{CI}$) |
...
| **Combined (Mean $\pm$ SD)** | Multi-Center | $509$ | $411$ | $920$ | **`0.7191 ± 0.0814`** | `[PENDING: sealed recompute]` |
...
- **Axis B2**: Gain over demographic Age baseline is designated as `[PENDING: sealed recompute]`.
...
Auditing the raw `val.tsv` and `train.tsv` files revealed three separate text parsing artifacts:
...
2. **Locked-Specification Implementation Deviation (Pattern 14)**: While the preregistration specification required Implicit Function Theorem (IFT) exact analytical backpropagation, the actual code unrolled 100 forward solver iterations into the autograd computation graph.
3. **Evidence Inflation from Stream Truncation (Precursor to Pattern 13)**: Earlier audit logs claimed 100% SHA-256 hash identity between verification runs (`run2` vs `run3`), which was subsequently traced to prematurely truncated 108-line log files. Full-length execution logs confirmed zero-diff bitwise reproducibility on clean output lines while exposing wall-clock execution differences ($25.5\text{ s}$ retrain duration).
