...
3. **Empirical Benchmarking Case Studies**: Uncovering sequence length bias, species distribution invariance, FASTA parsing artifacts, multi-site clinical degradation, and audit log immutability lessons.
...
### 4.1 Sequence Length Confound, Species Invariance, and Independent Annotation Bias in LLPS Benchmarks (Task B)
...
- **Validation Split ($n=656$ valid headers)**: 73.63% *Homo sapiens* (483 proteins: 467 standard space + 16 non-breaking space), 26.37% non-human across 17 model organisms (*S. cerevisiae* 5.9%, *C. reinhardtii* 3.0%, *M. tuberculosis* 2.6%, *X. laevis* 2.3%, *D. melanogaster* 2.1%, *R. norvegicus* 2.1%, *M. musculus* 1.2%, etc.).
- **Training Split ($n=2539$ valid headers)**: 51.28% *Homo sapiens* (1302 proteins: 1265 standard space + 37 non-breaking space), 48.72% non-human across 32 model organisms (SARS-CoV-2 9.97%, *E. coli* 4.81%, *C. elegans* 4.14%, Yeast 3.66%, *M. musculus* 2.72%, *D. melanogaster* 2.25%, etc.).

Evaluating species assignment itself as a baseline feature (`CONF_species`, binary `Is_Human` predictor vs phase separation label) yielded AUROCs of **`0.4872` [0.4400, 0.5344]** on validation ($n=656$) and **`0.4953` [0.4711, 0.5194]** on training ($n=2539$), establishing that species identity is uninformative noise ($0.50 \in \text{CI}$). 

Crucially, despite the 22.35%p species distribution shift between training (51.28% human) and validation (73.63% human), sequence length bias remains **strictly invariant** ($0.6010 \approx 0.6017$), proving that sequence length bias is an intrinsic, organism-independent artifact of the benchmark architecture.

| Dataset Split Evaluated | Target Feature Evaluated | Sample Size ($n$) | Positives ($y=1$) | Negatives ($y=0$) | AUROC | 95% Confidence Interval (Hanley-McNeil / Bootstrap) | Confound Significance |
| :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **Validation Split (`val.tsv`)** | Pure AA Sequence Length (`CONF_seqlen`) | $656$ | $441$ ($67.2\%$) | $215$ ($32.8\%$) | **`0.6017`** | `[0.5569, 0.6465]` (SE = $0.0229$) | **Statistically Significant** |
| **Validation Split (≥30 AA)** | Sensitivity Subset (Length $\ge 30$) | $648$ | $437$ ($67.4\%$) | $211$ ($32.6\%$) | **`0.5996`** | `[0.5544, 0.6448]` (SE = $0.0230$) | **Statistically Significant** |
| **Validation Split (`val.tsv`)** | FASTA Header Text Length (`CONF_header`) | $656$ | $441$ ($67.2\%$) | $215$ ($32.8\%$) | **`0.5762`** | `[0.5306, 0.6218]` (SE = $0.0232$) | **Statistically Significant** |
| **Validation Split (`val.tsv`)** | Missing Sequence Indicator (`CONF_missing`) | $697$ | $465$ ($66.7\%$) | $232$ ($33.3\%$) | **`0.4892`** | `[0.4436, 0.5348]` (SE = $0.0232$) | Uninformative Noise ($0.50 \in \text{CI}$) |
| **Validation Split (`val.tsv`)** | Species Identity (`CONF_species`) | $656$ | $441$ ($67.2\%$) | $215$ ($32.8\%$) | **`0.4872`** | `[0.4400, 0.5344]` (SE = $0.0241$) | Uninformative Noise ($0.50 \in \text{CI}$) |
| **Training Split (`train.tsv`)** | Pure AA Sequence Length (`CONF_seqlen`) | $2539$ | $1734$ ($68.3\%$) | $805$ ($31.7\%$) | **`0.6010`** | `[0.5781, 0.6239]` (SE = $0.0117$) | **Statistically Significant** |
| **Training Split (`train.tsv`)** | Species Identity (`CONF_species`) | $2539$ | $1734$ ($68.3\%$) | $805$ ($31.7\%$) | **`0.4953`** | `[0.4711, 0.5194]` (SE = $0.0123$) | Uninformative Noise ($0.50 \in \text{CI}$) |
