...
This demonstrates a statistically significant taxonomic interaction in the training split ($\Delta = 0.0535$, $\text{SE}_{\text{diff}} = 0.0236$, minimum detectable difference $\text{MDD} = 0.0465$, $p < 0.05$), where sequence length bias is accentuated in human proteins ($0.6291$) relative to non-human model organisms ($0.5756$). In the smaller validation split, human ($0.5948$) and non-human ($0.6076$) subsets exhibit overlapping confidence intervals. The findings show that sequence length bias is pervasive across all examined taxonomic strata.
...
| **Validation Strata: Human** | `Homo sapiens` Subset (`CONF_seqlen`) | $483$ | $321$ ($66.5\%$) | $162$ ($33.5\%$) | **`0.5948`** | `[0.5442, 0.6481]` (Bootstrap SE = $0.0264$) | **Statistically Significant** |
...
### 5.4 Case Study 4: Numerical Provenance Mapping and Verification Engine Self-Audit
During manuscript compilation, four subgroup AUROC values were initially reported with inverted taxonomic labels prior to log cross-checking. An initial verification script failed to detect the mismatch because it only verified whether numbers appeared somewhere in the text rather than enforcing line-by-line matching against target execution logs. This meta-audit failure led to building an uncompromised automated provenance engine (`build_automated_number_provenance_canonical.py`) that strictly extracts point estimates from manuscript markdown and enforces line-by-line string matching against execution logs (`canonical_paper1_audit_log.log`, `task-863.log`).
...
7. **Automated Log-Provenance Enforcement**: All quantitative values in reports and manuscripts must be parsed directly from manuscript markdown and verified line-by-line against raw execution log files using point-estimate string matching.
