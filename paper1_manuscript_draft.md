...
To test whether sequence length bias varies across taxonomic boundaries, we performed a stratified interaction audit across species strata within both splits (immutable log `task-1262.log`):
- **Training Split (`train.tsv` $n=2539$)**:
  - `Homo sapiens` Subset ($n=1302$): AUROC = **`0.6291`** [0.5977, 0.6614] (Bootstrap SE = $0.0164$)
  - Non-Human Subset ($n=1237$): AUROC = **`0.5756`** [0.5423, 0.6089] (Bootstrap SE = $0.0170$)
  - Viral `SARS-CoV-2` Subset ($n=253$): AUROC = **`0.5839`** [0.5057, 0.6599] (Bootstrap SE = $0.0387$)
- **Validation Split (`val.tsv` $n=656$)**:
  - `Homo sapiens` Subset ($n=483$): AUROC = **`0.5948`** [0.5442, 0.6502] (Bootstrap SE = $0.0264$)
  - Non-Human Subset ($n=173$): AUROC = **`0.6076`** [0.5151, 0.6985] (Bootstrap SE = $0.0454$)

This demonstrates a statistically significant taxonomic interaction in the training split ($\Delta = 0.0535$, $\text{SE}_{\text{diff}} = 0.0236$, minimum detectable difference $\text{MDD} = 0.0463$, $p < 0.05$), where sequence length bias is accentuated in human proteins ($0.6291$) relative to non-human model organisms ($0.5756$). In the smaller validation split, human ($0.5948$) and non-human ($0.6076$) subsets exhibit overlapping confidence intervals. The findings show that sequence length bias is pervasive across all examined taxonomic strata.
...
| **Validation Strata: Human** | `Homo sapiens` Subset (`CONF_seqlen`) | $483$ | $321$ ($66.5\%$) | $162$ ($33.5\%$) | **`0.5948`** | `[0.5442, 0.6502]` (Bootstrap SE = $0.0264$) | **Statistically Significant** |
| **Validation Strata: Non-Human** | Non-Human Subset (`CONF_seqlen`) | $173$ | $120$ ($69.4\%$) | $53$ ($30.6\%$) | **`0.6076`** | `[0.5151, 0.6985]` (Bootstrap SE = $0.0454$) | **Statistically Significant** |
...
| **Training Strata: Viral** | SARS-CoV-2 Subset (`CONF_seqlen`) | $253$ | $180$ ($71.1\%$) | $73$ ($28.9\%$) | **`0.5839`** | `[0.5057, 0.6599]` (Bootstrap SE = $0.0387$) | **Statistically Significant** |
...
### 5.4 Case Study 4: Numerical Provenance Mapping, Recurrent Drift, and Audit Engine Circular Verification
During manuscript compilation, four subgroup AUROC values were initially reported with inverted taxonomic labels prior to log cross-checking. A subsequent attempt to build an automated verification engine resulted in circular verification: the script generated a new log file within the same process cycle as inspecting it, while failing to enforce that point estimates must strictly fall within their own confidence intervals ($lo \le pval \le hi$). This meta-audit failure led to establishing strict immutability rules: verification scripts must never generate or overwrite audit logs, execution logs must be anchored by external SHA-256 hashes at creation time, and verification inspectors must enforce point-in-CI mathematical sanity constraints ($lo \le pval \le hi$).
...
7. **Strict Log Immutability and Mathematical Sanity Checks**: Verification engines must never create or overwrite execution logs, must verify against immutable historical log files anchored by SHA-256 hashes, and must enforce mathematical point-in-CI sanity constraints ($lo \le pval \le hi$).
