...
# Subplot 1: Monotone DEQ (N=1 Banach Collapse)
...
ax1.set_xlabel(r'$\phi$ (degrees)', fontsize=12)
ax1.set_ylabel(r'$\psi$ (degrees)', fontsize=12)
ax1.set_title('Monotone DEQ (Banach Contraction: Lip ≤ 0.788 < 1)\nN=1 Single Fixed-Point Collapse', fontsize=12, pad=10)
...
ax2.set_xlabel(r'$\phi$ (degrees)', fontsize=12)
ax2.set_ylabel(r'$\psi$ (degrees)', fontsize=12)
...
out_file = os.path.join("paper3_iclr", "fig3_monotone_vs_equiphase_v2.png")
plt.tight_layout()
plt.savefig(out_file, dpi=300)
plt.close()

# Quarantine old file
old_file = os.path.join("paper3_iclr", "fig3_monotone_vs_equiphase.png")
if os.path.exists(old_file):
    shutil.move(old_file, os.path.join("quarantine", "fig3_monotone_vs_equiphase_v1.png"))

h_val = hashlib.sha256(open(out_file, "rb").read()).hexdigest().upper()
print(f"Fig 3 v2 generated successfully at {out_file} | SHA256={h_val}")
