...
# Move v2 figures to quarantine
for v2_file in ["fig1_ala2_density_attractors_v2.png", "fig3_monotone_vs_equiphase_v2.png"]:
    src = os.path.join(paper3_dir, v2_file)
    if os.path.exists(src):
        dst = os.path.join(quarantine_dir, v2_file)
        shutil.move(src, dst)
        print(f"Quarantined {v2_file} -> {dst}")
